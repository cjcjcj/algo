...
func BenchmarkChineseBuild(b *testing.B) {
	...
	for _, bm := range benchmarks {
		...
		b.Run(name, func(b *testing.B) {
			for i := 0; i < b.N; i++ {
				st.NewCHSeg(nums, sum)
			}
		})
	}
}

func BenchmarkChineseUpdate(b *testing.B) {
	rand.Seed(time.Now().Unix())
	...
	for _, bm := range benchmarks {
		nums := rand.Perm(bm.size)
		name := strconv.Itoa(bm.size)
		s := st.NewCHSeg(nums, sum)

		b.Run(name, func(b *testing.B) {
			for i := 0; i < b.N; i++ {
				s.Update(0, rand.Intn(bm.size), rand.Int())
			}
		})
	}
}

func BenchmarkChineseQuery(b *testing.B) {
	rand.Seed(time.Now().Unix())
